...
export const requestBudgetData = () => {
    let dataFromResponseOfAxiosRequest = axios.get('/api/budget-data').then(respose => respose.data)

    return {
        type: REQUEST_BUDGET_DATA, 
        payload: dataFromResponseOfAxiosRequest
    }
}

export const addPurchase = (price, description, category) => {
    let newPurcahse = axios.post('/api/budget-data/purchase', {
        description, 
        price, 
        category
    }).then(res => (
